...
var computersElement = document.getElementById('computer-div');
        ...
        setTimeout(function () {
            ...
            for ( var i = 0; i < computersSelected.length; i++) {
                ...
                computersElementHTML += "<div class='computer-card-container' >" +
                   ...
                   c.id + ")'>Добавить в корзину</button></div>" +
                   "</div></div></div>";
            }
...
function refreshComputersBasket() {
    ...
    for (let index = 0; index < basketComputers.length; index++) {
        ...
        basketComputersTableBodyElementHtml += ' AZN</td><td><input min="1" max="10000" type="number"' +
           b.count + '" ' +
...
calculateBasketTotalPrice();
function computerCoutChanged(countInput, computerId) {
    if (countInput.value == '' || countInput.value == '0') { countInput.value = "1";}
    for (let index = 0; index < basketComputers.length; index++) {
        const b = basketComputers[index];
        if (b.computer.id === computerId) {
            b.count = Number(countInput.value);
            document.getElementById('computer-total-price-' + b.computer.id).innerHTML = 
             "" + (b.count * b.computer.price) + " AZN";
            break;
        }
    }
    localStorage.setItem('basketComputers', JSON.stringify(basketComputers));
...
function endOrderRegistration(customer) {
    ...
    closeConfirmOrder();
    setTimeout(() => {
        showAlertMessage("Ваш заказ принят!", 2000);
    }, 1000);
...
function onSearchKeyDown(event) {
    if (event.keyCode == 13) {
        ...
        setTimeout(function () {
            ...
            computersElement.style.display = 'block';
            if (findedComputers.length == 0) { computersElement.innerHTML = "<h2 class='not-found'>По вашему запросу, ноутбук не найден</h2>"}
            allComputersLoaded = false;
        }, 500);
...
window.addEventListener('scroll', () => {
    if (allComputersLoaded) {
        ...
        if (allowScroll) {
            const distanceToBottom = document.body.getBoudingClientRect().bottom;
